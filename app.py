...
DB_PATH=os.getenv('DB_PATH','data/quantum_entry.db'); Path(DB_PATH).parent.mkdir(parents=True,exist_ok=True)
...
class Binance:
    def __init__(self):self.s=None;self.sem=asyncio.Semaphore(MAX_CONCURRENCY)
    async def start(self):self.s=aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=20))
            ...
            for i in range(4):
                try:
                    async with self.s.get(BINANCE_BASE+path,params=params) as r:
                        if r.status in (418,429):await asyncio.sleep(2**i+1);continue
                        r.raise_for_status();return await r.json()
                except Exception:
                    if i==3:raise
                    await asyncio.sleep(1.4**i)
    async def symbols(self):
        d=await self.get('/fapi/v1/exchangeInfo');return [x['symbol'] for x in d['symbols'] if x.get('status')=='TRADING' and x.get('contractType')=='PERPETUAL' and x.get('quoteAsset')=='USDT']
    async def tickers(self):return await self.get('/fapi/v1/ticker/24hr')
    async def klines(self,s,i,l=100):return await self.get('/fapi/v1/klines',{'symbol':s,'interval':i,'limit':l})
    async def oi(self,s):return await self.get('/futures/data/openInterestHist',{'symbol':s,'period':'5m','limit':12})
    async def depth(self,s):return await self.get('/fapi/v1/depth',{'symbol':s,'limit':100})
    async def premium(self,s):return await self.get('/fapi/v1/premiumIndex',{'symbol':s})
...
class Engine:
    def __init__(self):self.b=Binance();self.ts=None;self.running=True;self.scan_no=0;self.last_scan=None;self.last_error=None;self.symbols=0;self.candidates=0;self.alerts=0;self.fast={};self.hist={};self.mood=50
    async def send(self,text):
        ...
        try:
            async with self.ts.post(f'https://api.telegram.org/bot{BOT_TOKEN}/sendMessage',json={'chat_id':CHAT_ID,'text':text,'parse_mode':'HTML','disable_web_page_preview':True},timeout=20) as r:
                if r.status!=200:log.error('Telegram %s %s',r.status,await r.text());return False
                return True
        except Exception:log.exception('telegram');return False
    async def start(self):
        await init_db();await self.b.start();self.ts=aiohttp.ClientSession()
        if SEND_STARTUP_MESSAGE:await self.send(f'✅ <b>Ahmed Quantum Entry AI بدأ العمل</b>\n\n🎛️ الوضع: <b>{ENTRY_MODE}</b>\n🧠 Pre-Explosion + Order Flow First-Reaction\n⏰ 1M/3M للتوقيت — 15M/1H/4H للسياق')
    ...
    async def loop(self):
        while self.running:
            ...
            try:al,an=await self.scan();self.last_error=None
            except Exception as e:err=repr(e);self.last_error=err;log.exception('scan')
    ...
    async def scan(self):
        ...
        rank.sort(reverse=True);cand=[x[2] for x in rank[:RADAR_POOL][:DEEP_CANDIDATES]];self.candidates=len(cand);res=await asyncio.gather(*[self.analyze(s) for s in cand],return_exceptions=True);al=an=0
        for r in res:
    ...
    async def analyze(self,symbol):
        k1,k3,k15,k1h,k4h,oi,depth,prem=await asyncio.gather(self.b.klines(symbol,'1m'),self.b.klines(symbol,'3m'),self.b.klines(symbol,'15m'),self.b.klines(symbol,'1h'),self.b.klines(symbol,'4h'),self.b.oi(symbol),self.b.depth(symbol),self.b.premium(symbol));out=[]
    ...
    async def track(self):
        while self.running:
            try:
                ...
                prices={x['symbol']:float(x['price']) for x in await self.b.get('/fapi/v1/ticker/price') if 'symbol'in x and 'price'in x}
                async with aiosqlite.connect(DB_PATH) as db:
                    for r in rows:
...
@app.get('/health')
async def health():return {'ok':engine.last_error is None,'service':'Ahmed Quantum Entry AI','entry_mode':ENTRY_MODE,'last_scan':engine.last_scan,'last_error':engine.last_error,'scan_number':engine.scan_no,'symbols':engine.symbols,'candidates':engine.candidates,'alerts':engine.alerts,'market_mood':engine.mood,'time':now().isoformat()}
